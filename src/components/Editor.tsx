...
const Editor = forwardRef<EditorHandle, EditorProps>(
  ({hatToAdd, onHatAdded, onImageStateChange}, ref) => {
      ...
      setHats((prev) =>
        prev.map((hat) => {
          ...
          if (mode === "ROTATE") {
            ...
            return {
              ...hat,
              rotation: startInteraction.startRotation + (angle - startAngle),
            };
          }

          if (mode === "SCALE") {
            const dist = Math.sqrt(
              Math.pow(x - hat.x, 2) + Math.pow(y - hat.y, 2)
            );
            const startDist = Math.sqrt(
              Math.pow(startInteraction.mouseX - hat.x, 2) +
                Math.pow(startInteraction.mouseY - hat.y, 2)
            );
